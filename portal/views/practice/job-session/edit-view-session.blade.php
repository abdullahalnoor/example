...
@section('content')
    ...
    <section class="content">
      ...
      <div class="row">      
       <div class="col-md-12">
        <table class="text-center table table-responsive">
          ...
          <tr >
            <td id="hours"></td>
            <td>{{ $updateSession['start_time']}}-{{ $updateSession['end_time']}}</td>
            <td>{{$updateSession['unpaid_break_hr']}}: {{$updateSession['unpaid_break_min']}}</td>
            <td>{{$billableHours}} : {{$billableMinute}}</td>
            <td>${{ $updateSession['hourly_rate']}}</td>
            <td>{{$total}}</td>
            <td>
      ...
      <div class="row">
        <div class="col-md-12">
          <a href="{{url('practice/edit-job-session/'.Session::get('id'))}}" class="btn btn-warning">Back</a>   
          <a href="{{url('practice/edit-job-session-view/'.Session::get('id'))}}" class="btn btn-primary pull-right">Next</a>      
